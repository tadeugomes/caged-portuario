...
```{r fig-saldo-ma-alternativo, warning=FALSE, message=FALSE}
...
df_tres %>%
  ...
  pivot_wider(
    ...
    values_fill = list(contagem = 0)  # Preenche com 0 se não houver valor para um grupo
  ) %>%
  mutate(saldo = Admitidos - Desligados) %>%  # Calcula o saldo
  pivot_longer(
    cols = c("Admitidos", "Desligados", "saldo"),
    names_to = "Movimentacao",
    values_to = "valor"
  ) %>%
  ggplot(aes(x = data, y = valor, color = Movimentacao)) +
  geom_line(size = 1, aes(linetype = Movimentacao)) +
  labs(
