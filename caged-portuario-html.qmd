...
format:
  html: 
    ...
    echo: false               # Não exibe os comandos no documento final
    number-sections: true     # Numeração das seções
    lang: pt                  # Idioma do documento
    theme: cosmo             # Tema do documento (opcional: flatly, united, cosmo, etc.)
    code-fold: true           # Permite dobrar e expandir os blocos de código (opcional)
    css: "styles.css"         # Arquivo CSS customizado (opcional)
    df-print: paged           # Estilo de impressão de dataframes (opcional)
    self-contained: true # Adiciona as imagens diretamente ao arquivo HTML
---
...
```
```{r}
# Define o estilo personalizado op_style (observatorio portuario)
op_style <- function() {
  list(
    labs(
      title = "",
...
# Obter os dados para o último mês disponível
dados <- get_data()

# Criar gráfico com ggplot2
ggplot(dados, aes(x = reorder(sigla_uf, saldo_somado), y = saldo_somado)) +
  geom_bar(stat = "identity", fill = "#87CEFA") +
  coord_flip() + # Inverte os eixos para melhor visualização
  geom_text(aes(label = saldo_somado), 
            hjust = -0.3, # Ajusta a posição horizontal do texto
            color = "black", 
            size = 3.5) + 
  op_style()
...
quartis <- quantile(salario_regiao$salario_mensal, probs = c(0.25, 0.75))
...
# Calcula o saldo total de movimentação por cbo_2002 e seleciona os 10 maiores
...
ggplot(top_10_plot_data, aes(x = reorder(cbo_2002, n), y = n, fill = saldo_movimentacao)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 8), # Tamanho do texto do eixo X
    axis.text.y = element_text(size = 8), # Tamanho do texto do eixo Y
    plot.margin = margin(1, 1, 1, 2, "cm") # Ajusta as margens para evitar o truncamento
  )
...
```{r}
#| fig-width: 7  # Aumenta a largura da figura
#| fig-height: 5 # Aumenta a altura da figura
#| warning: false
#| message: false
...
ggplot(bottom_10_plot_data, aes(x = reorder(cbo_2002, n), y = n, fill = saldo_movimentacao)) +
  ...
  geom_text(aes(label = n), 
            position = position_dodge(width = 0.8), # Ajusta a posição do texto para barras lado a lado
            hjust = -0.3, # Ajusta a posição horizontal do texto para fora das barras
            size = 3, # Tamanho do texto
            color = "black") + # Cor do texto
 op_style()+
  theme(
...
dados_selecionados <- dados_ma_pivot %>%
  ...
  arrange(desc(data)) # Ordena os meses de forma decrescente

# Formatação da data para exibir apenas mês e ano
dados_selecionados$data <- format(dados_selecionados$data, "%b %Y")

# Cria a tabela estática usando flextable
tabela <- flextable(dados_selecionados) %>%
  set_header_labels(
    data = "Mês",
  ...
  theme_box() %>%
  autofit() %>%
  color(i = ~ saldo < 0, color = "red", part = "body") %>%
  color(i = ~ saldo > 0, color = "black", part = "body") %>%
  bold(j = ~ saldo, part = "body")
...
#| fig.width: 7  # Aumenta a largura da figura
#| fig.height: 5 # Aumenta a altura da figura
#| fig-cap: "10 ocupações com saldo negativo no mês no Maranhão"
