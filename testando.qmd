...
ggplot(df_saldo_mensal, aes(x = reorder(sigla_uf, saldo_somado_mensal), y = saldo_somado_mensal)) +
  ...
  theme_minimal() +
  geom_text(aes(label = saldo_somado_mensal), hjust = -0.3)
...
A movimentação de empregos portuários e aquaviários no estado do Maranhão no mês resultou no saldo de `r saldo_ma` vagas, uma variação de, 
