...
# 1. Primeiro, verifique se os dados existem e não estão vazios
if (!exists("ultimo_dado") || nrow(ultimo_dado) == 0) {
  stop("Dados não disponíveis ou vazios")
}

# 2. Criar função auxiliar para calcular o saldo
calcular_saldo <- function(dados) {
  dados %>%
    ...
    mutate(
      Admitidos = if_else(is.na(Admitidos), 0L, Admitidos),
      Desligados = if_else(is.na(Desligados), 0L, Desligados),
      saldo = Admitidos - Desligados
    )
}

# 3. Aplicar a função com tratamento de erro
top_10_cbo <- tryCatch({
  resultado <- calcular_saldo(ultimo_dado) %>%
    arrange(desc(saldo)) %>%
    slice_head(n = 10)
  
  if (nrow(resultado) == 0) {
    stop("Nenhum resultado encontrado após o cálculo")
  }
  
  resultado
}, error = function(e) {
  message("Erro ao calcular top 10: ", e$message)
  return(NULL)
})

# 4. Verificar resultado antes de continuar
if (!is.null(top_10_cbo)) {
  print("Cálculo realizado com sucesso")
  print(dim(top_10_cbo))
}


# Transformar os dados para formato longo primeiro
top_10_plot_data <- top_10_cbo %>%
  pivot_longer(
    cols = c(Admitidos, Desligados),
    names_to = "tipo",
    values_to = "quantidade"
  )

# Criar o gráfico com barras lado a lado
ggplot(top_10_plot_data, aes(x = reorder(cbo_2002, saldo), y = quantidade, fill = tipo)) +
  ...
  theme(
    ...
    plot.margin = margin(r = 40)  # Adiciona margem à direita para os rótulos
  )
...
# Filtra os dados para o estado do Maranhao (sigla_uf == "MA")
dados_ma <- df_tres %>%
  filter(sigla_uf == "MA")

# 2. Count 'Admitidos' and 'Desligados' grouped by 'data'
dados_ma_agrupado <- dados_ma %>%
  group_by(data, saldo_movimentacao) %>%
  summarise(contagem = n(), .groups = 'drop')

# 3. Pivot data to wider format
...
# 4. Calculate net movement (saldo)
dados_ma_pivot <- dados_ma_pivot %>%
  mutate(saldo = Admitidos - Desligados)

# 5. Identifica os últimos valores de cada série
dados_finais <- dados_ma_pivot %>%
...
# Cria o grafico de linhas com admitidos, desligados e saldo
p_2 <- ggplot(dados_ma_pivot, aes(x = data)) +
...
dados_selecionados <- dados_ma_pivot %>%
  filter(data >= max(data) %m-% months(10)) %>%
  select(data, Desligados, Admitidos, saldo)
